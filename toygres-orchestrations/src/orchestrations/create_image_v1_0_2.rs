//! Create Image orchestration v1.0.2
//!
//! Changes from v1.0.1:
//! - Fixed: replaced std::time::Instant::now() with ctx.utc_now() for deterministic replay
//! - Fixed: replaced std::env::var() with get-storage-config activity for deterministic replay
...
use crate::activity_types::{
    ...
    GetStorageConfigInput, GetStorageConfigOutput,
};
use crate::types::{CreateImageInput, CreateImageOutput};

pub async fn create_image_1_0_2_orchestration(
    ...
    ctx.set_custom_status("Initializing backup");
    ...
    ctx.set_custom_status("Validating source instance");
    ...
    let source_password = match &input.source_password {
        ...
        _ => {
            ctx.set_custom_status("Fetching instance password");
            ctx.trace_info("Fetching password from K8s Secret");
            get_password_from_secret(&ctx, &input.source_k8s_name, &namespace).await?
        }
    };

    // Step 1c: Get storage configuration via activity (deterministic on replay)
    let storage_config: GetStorageConfigOutput = ctx
        .schedule_activity_typed::<GetStorageConfigInput, GetStorageConfigOutput>(
            activities::get_storage_config::NAME,
            &GetStorageConfigInput {},
        )
        .await?;

    let blob_storage_account = storage_config.storage_account;
    let blob_container = storage_config.container;
    ...
    ctx.set_custom_status("Creating image record");
    ...
    ctx.set_custom_status("Starting backup job");
    ...
    match run_backup(&ctx, &input, &job_name, &namespace, &instance, &blob_storage_account, &blob_container, &blob_path, &source_password).await {
        Ok((backup_size, checksum)) => {
            ctx.set_custom_status("Updating image to ready");
            ...
            ctx.set_custom_status("Cleaning up backup job");
            ...
            let elapsed = end_time.duration_since(start_time)
                .map(|d| d.as_secs())
                .unwrap_or(0);
            ctx.set_custom_status(&format!("Completed in {}s", elapsed));
...
async fn run_backup(
    ...
    loop {
        ...
        ctx.set_custom_status(&format!("Backup running ({}s elapsed)", elapsed.as_secs()));
